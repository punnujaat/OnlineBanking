...
public static boolean checkLogin(int accountno,String username,String password){
	...
	try {
		...
		PreparedStatement ps=con.prepareStatement("Select * from NEWACCOUNT.raft where AccountNo=? and UserName = ? and Password =?");
...
public static boolean checkLogin1(String username,String password){
	boolean status=false;
	Connection con=GetCon.getCon();
	try {
		//PreparedStatement ps=con.prepareStatement("Select * from MAILCASTINGUSER where EMAILADD = ? and PASSWORD =?");
